...
void loop (){
  ...
  //if((int)pos>0) //DESCOMENTAR PARA TER O FUNCIONAMENTO ESPERADO
  {
    if(GETSMS_AUTH_SMS == sms.GetAuthorizedSMS((int)pos, number, message, 50, 1,3)){
      ...
      //Defini o conteúdo da stringmessage como vazio.
      message[0]='\0';
      ...
      if(!a1 && !a2 && !b1 && !b2 && !c1 && !c2 && !d1 && !d2)
      {
        ...
        ositema envia uma mensagem de segurança ao número proprietário.*/
  }
  //else
  {
    ...
    Serial.println("+556193697829");
    //Função padrão para envio de sms. (numero de destino, string com texto)
    //error=sms.SendSMS("+556186274372", security);
    error=sms.SendSMS("+556291353328", "Teste 123 da silva");
